...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):  # sources=0
        self.mode = 'stream'
        self.img_size = img_size
        self.stride = stride
        ...
        for i, s in enumerate(sources):
            ...
            try:
                cap = cv2.VideoCapture(url)  # 打开摄像头这里
                ...
                self.imgs[i] = np.asanyarray(aligned_frames.get_color_frame().get_data())  # guarantee first frame
                thread = Thread(target=self.update_deep, args=([i, pipeline, align]), daemon=True)
                thread.start()
                # cv2.imshow('s', color_frame)
                # cv2.waitKey(1)
    ...
    def update(self, index, cap):
        ...
        while cap.isOpened():
            ...
            if n == 4:  # read every 4th frame
                success, im = cap.retrieve()  # cap.read()函数其实是grab()和retireve()的组合
    ...
    def update_deep(self, index, pipeline, align):
        n = 0
        # # pipeline = rs.pipeline()
        # config = rs.config()
        # config.enable_stream(rs.stream.color, 640, 480, rs.format.bgr8, 15)
        ...
        while True:
            ...
            if n == 4:
                frames = pipeline.wait_for_frames()
                aligned_frames = align.process(frames)
                self.imgs[index] = np.asanyarray(aligned_frames.get_color_frame().get_data())
                n = 0
            try:
                time.sleep(1 / self.fps)  # wait time
            except:
